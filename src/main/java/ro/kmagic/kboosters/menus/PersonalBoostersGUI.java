...
public class PersonalBoostersGUI {

    private final PaginatedGui gui;

    public PersonalBoostersGUI(Player player) {
        Boosters instance = Boosters.getInstance();
        ConfigurationSection menuSection = instance.getConfig().getConfigurationSection("personal-boosters-menu");
        ...
        for(Booster booster : instance.getPlayerManager().getPersonalBoosters(player.getUniqueId()).stream().sorted((a, b) -> {
            ...
            name = name.replace("{name}", instance.getConfigBoosterManager().getBooster(booster.getID()).getName())
                    .replace("{multiplier}", "" + instance.getConfigBoosterManager().getBooster(booster.getID()).getMultiplier());
            ...
            GuiItem item = ItemBuilder.from(new MaterialData(id).getItemType())
                    ...
                    .asGuiItem(event -> {
                        if(instance.getPlayerManager().getPlayer(event.getWhoClicked().getUniqueId()).getActiveBoosters().stream()
                                .anyMatch(b -> instance.getConfigBoosterManager().getBooster(b.getID()).getType() == BoosterType.PERSONAL
                                        && instance.getConfigBoosterManager().getBooster(b.getID()).getServer().equalsIgnoreCase(instance.getConfig().getString("server")))) {
                            event.getWhoClicked().sendMessage(Utils.color(instance.getConfig().getString("messages.booster-already-active")));
